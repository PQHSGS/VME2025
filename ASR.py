...
def load_asr_model():
    ...
    try:
        ...
        asr_model = WhisperModel(model_path, device="cuda", compute_type="float32")
...
def transcribe_audio():
    ...
    try:
        ...
        transcription_result, _ = asr_model.transcribe(
            audio_data,
            beam_size=5,                      # more exploration for tricky speech
            temperature=0.2,                  # allow slight diversity
            no_speech_threshold=0.55,         # less strict, keep short utterances
            hallucination_silence_threshold=2.0, 
            log_prob_threshold=-0.5,          # filter very unlikely tokens
            language='vi'
        )
